...
for m_l in data['m_luft']:
    for m_m in data['m_mischung']:
        ...
        nw.solve('design')
        x_aches.append(str(round(m_l)) + " - " +  str(round(m_m)))
        x_aches_luft.append(round(m_l))
        x_aches_misch.append(round(m_m))
...
df = pd.DataFrame({
    ...
    'm7': x_aches_misch,
    'T13': T_13['m_mischung'],
    'Netto-P': net_power['m_mischung']
})

excel_file_path = 'C:/TU-Berlin/01_Masterarbeit/Bilder_tespy/data_frame_iteration.xlsx'
