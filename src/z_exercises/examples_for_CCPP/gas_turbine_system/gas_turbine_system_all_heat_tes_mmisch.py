...
plt.rc('font', **{'size': 18})
data = {
    'm_luft': np.linspace(800, 800, 1),
    'm_mischung': np.linspace(100, 200, 11)
}
...
for m_l in data['m_luft']:
    for m_m in data['m_mischung']:
        ...
        nw.solve('design')
        x_aches.append(str(round(m_m)) )
        T_13['m_mischung'] += [round(m_tit_to_exp.T.val,2)]
        net_power['m_mischung'] += [round(-generator.P.val/1000000,2)]


fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8))

# Plot on the first subplot (x, y1)
ax1.plot(x_aches, T_13['m_mischung'], 'b-', label='T13',marker='x', linewidth=2)
...
ax2.plot(x_aches, net_power['m_mischung'], 'r-', label='Netto-P',marker='o', linewidth=2)
...
fig.text(0.5, 0.02, 'Teilluftstrom (7)', ha='center', fontsize=12)
fig.suptitle('Konstanter angesaugter Luftstrom (1) = 800 kg/s')

plt.savefig('C:/TU-Berlin/01_Masterarbeit/Bilder_tespy/teilmassenstrom_T_P.png')
plt.close()
