...
heat_in_B.add_comps({'comp': so_h, 'base':'bus'})

heat_out_B = Bus('Heat Out')
heat_out_B.add_comps({'comp':si_h})

heated_in_B = Bus('Heated In')
heated_in_B.add_comps({'comp':so_c, 'base':'bus'})

heated_out_B = Bus('Heated out')
heated_out_B.add_comps({'comp': si_c})


# ++ add busses to network
nw.add_busses(heat_in_B, heat_out_B, heated_in_B, heated_out_B)

# define ena and assign as P, F, L
ean = ExergyAnalysis(nw, E_F=[heat_in_B, heat_out_B], E_P=[heated_in_B, heated_out_B], E_L=[])
