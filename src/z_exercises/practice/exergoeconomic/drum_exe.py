...
so_eva = Source("from eva")
si_sat_g = Sink("sat gas")

# connections
liq_in_2_hx = Connection(so_eco, 'out1', hx, 'in1', label='from economiser to drum')
gas_in_2_hx = Connection(so_eva, 'out1', hx, 'in2', label='from evaporator to drum')
...
eva_out = Bus('gas in')
eva_out.add_comps({'comp': so_eva, 'base': 'bus'})
...
nw.add_busses(eco_out, liq_out, eva_out, gas_out)
